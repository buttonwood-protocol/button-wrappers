...
task('deploy:InceptionTokenOracle:prefilled', 'Verifies on etherscan')
  .addParam('underlying', 'the underlying token name', undefined, types.string, false)
  .setAction(
...
task('deploy:InceptionTokenOracle')
  .addParam('inceptionvault', 'the InceptionVault address', undefined, types.string, false)
  .setAction(async function (args: TaskArguments, hre) {
    const { inceptionvault } = args;
    console.log('Signer', await (await hre.ethers.getSigners())[0].getAddress());
    const InceptionTokenOracle = await hre.ethers.getContractFactory('InceptionTokenOracle');
    const inceptionTokenOracle = await InceptionTokenOracle.deploy(inceptionvault);
    await inceptionTokenOracle.deployed();
    console.log(`inceptionTokenOracle deployed to ${inceptionTokenOracle.address}`);
